...
#define USE_ENTERPRISE_WIFI true  // Use WPA2 Enterprise for KMITL-Legacy

// WiFi และ MQTT Configuration
bool checkWifi = false;
const char* ssid = "KMITL-Legacy";
const char* password = "";  // Not used for enterprise

// ========== WiFi KMITL-Legacy (WPA2 Enterprise) ==========
...
const char* device_id = "ESP32_HVAC_001";
...
//------------------------------- WiFi -----------------------------------// ===== SCAN & CONNECT with WPA2 Enterprise + BSSID =====
bool scanAndConnect() {
  Serial.println("\n🔍 Scanning...");
  ...
  const char* target_ssid = USE_ENTERPRISE_WIFI ? enterprise_ssid : ssid;
  ...
  // Find the strongest AP with target SSID
  for (int i = 0; i < n; i++) {
    Serial.printf("  %d: %-25s (%d dBm) ch:%d\n",
      i + 1, WiFi.SSID(i).c_str(), WiFi.RSSI(i), WiFi.channel(i));

    if (WiFi.SSID(i) == String(target_ssid)) {
      // Select AP with best signal
      ...
      Serial.printf("  ^ ✅ TARGET FOUND! ch:%d BSSID: %02X:%02X:%02X:%02X:%02X:%02X\n",
        WiFi.channel(i), WiFi.BSSID(i)[0], WiFi.BSSID(i)[1], WiFi.BSSID(i)[2], 
  ...
  Serial.printf("📶 Best AP: ch:%d RSSI:%d BSSID: %02X:%02X:%02X:%02X:%02X:%02X\n",
    channel, bestRssi, bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5]);
  ...
  if (USE_ENTERPRISE_WIFI) {
    // Configure WPA2 Enterprise FIRST
    ...
    // Connect with BSSID and channel for reliability
    WiFi.begin(target_ssid, WPA2_AUTH_PEAP, eap_identity, eap_username, eap_password, NULL, NULL, NULL, channel, bssid, true);
  } else {
    // Regular WPA2 Personal
    WiFi.begin(target_ssid, password, channel, bssid, true);
  }
  ...
  for (int i = 0; i < 40; i++) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      esp_wifi_get_channel(&staChannel, &second);
      Serial.printf("   Channel: %d (AP synced)\n", staChannel);
...
//  WiFi.mode(WIFI_AP_STA);
//  
//  // ⚠️ เปลี่ยนจาก channel 1 เป็น channel 11 ให้ตรงกับ sender
//  WiFi.softAP("ESP32_ESPNOW", "", 6);  // เปลี่ยนเป็น channel 11
//  
//  Serial.println("✅ SoftAP started on channel 11");
//  
...
//    
//    // ⚠️ ถ้า WiFi router ไม่ใช่ channel 6 จะมีปัญหา
//    if (channel != 6) {
//      Serial.println("⚠️ WARNING: WiFi router channel mismatch!");
//      Serial.printf("   Router is on channel %d, but AP is on 6\n", channel);
//      Serial.println("   ESP-NOW may be unreliable!");
//    }
//  } else {
//    wifiConnected = false;
